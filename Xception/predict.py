...
def summary_table(matrix,num_classes,labels):
    ...
    with open('./Xception/table_net_test.txt', 'a+') as f:
        f.write(str(table))
    f.close()
    print(table)
...
def plot_conf(matrix,num_classes,labels):
    ...
    plt.tight_layout()
    plt.savefig('./Xception/conf_net_test.jpg')
...
def main():
    ...
    weights_path = "./Xception/xception.pth"
    ...
    print()
    print("总测试时间：", t2)
    with open('./Xception/acc_time_net_test.txt', 'a+', encoding='utf-8') as f:
