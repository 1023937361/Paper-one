...
def summary_table(matrix,num_classes,labels):
    ...
    with open('./Shufflenet_V2x05/table_net_test.txt', 'a+') as f:
        f.write(str(table))
    f.close()
    print(table)
...
def plot_conf(matrix,num_classes,labels):
    ...
    plt.tight_layout()
    plt.savefig('./Shufflenet_V2x05/conf_net_test.jpg')
...
def main():
    ...
    weights_path = "./Shufflenet_V2x05/shufflenet_v2_x0_5.pth"
    ...
    print()
    print("总测试时间：", t2)
    with open('./Shufflenet_V2x05/acc_time_net_test.txt', 'a+', encoding='utf-8') as f:
