...
def summary_table(matrix,num_classes,labels):
    ...
    with open('./Googlenet/table_net_val.txt', 'a+') as f:
        f.write(str(table))
    f.close()
    print(table)
...
def plot_conf(matrix,num_classes,labels):
    ...
    plt.tight_layout()
    plt.savefig('./Googlenet/conf_net_val.jpg')
...
def main():
    ...
    save_path = './Googlenet/googlenet.pth'#模型最终保存路径
    data_transform = {
    ...
    with open('./Googlenet/acc_time_net_val.txt', 'a+', encoding='utf-8') as f:
    ...
    plt.legend()
    plt.savefig('./Googlenet/acc_net.jpg')
    plt.show(block=False)
    plt.pause(5)
    plt.close()
    ...
    plt.legend()
    plt.savefig('./Googlenet/loss_net.jpg')
    plt.show(block=False)
    plt.pause(5)
    plt.close()
