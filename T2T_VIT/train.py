...
from T2T_VIT.t2t_vit import T2t_vit_7
...
def summary_table(matrix,num_classes,labels):
    ...
    with open('./T2T_VIT/table_net_val.txt', 'a+') as f:
        f.write(str(table))
    f.close()
    print(table)
...
def plot_conf(matrix,num_classes,labels):
    ...
    plt.tight_layout()
    plt.savefig('./T2T_VIT/conf_net_val.jpg')
...
def main():
    ...
    save_path = './T2T_VIT/t2t_vit.pth'#模型最终保存路径
    data_transform = {
    ...
    with open('./T2T_VIT/class_indices.json', 'w') as json_file:
    ...
    with open('./T2T_VIT/acc_time_net_val.txt', 'a+', encoding='utf-8') as f:
    ...
    plt.legend()
    plt.savefig('./T2T_VIT/acc_net.jpg')
    plt.show(block=False)
    plt.pause(5)
    plt.close()
    ...
    plt.legend()
    plt.savefig('./T2T_VIT/loss_net.jpg')
    plt.show(block=False)
    plt.pause(5)
    plt.close()
