...
def summary_table(matrix,num_classes,labels):
    ...
    with open('./Deit/table_net_val.txt', 'a+') as f:
        f.write(str(table))
    f.close()
    # print(table)
...
def plot_conf(matrix,num_classes,labels):
    ...
    plt.tight_layout()
    plt.savefig('./Deit/conf_net_val.jpg')
...
def main():
    ...
    save_path = './Deit/deit.pth'#模型最终保存路径
    data_transform = {
    ...
    with open('./Deit/class_indices.json', 'w') as json_file:
    ...
    with open('./Deit/acc_time_net_val.txt', 'a+', encoding='utf-8') as f:
