...
def summary_table(matrix,num_classes,labels):
    ...
    with open('./Mobilenet_V2/table_net_val.txt', 'a+') as f:
        f.write(str(table))
    f.close()
    print(table)
...
def plot_conf(matrix,num_classes,labels):
    ...
    plt.tight_layout()
    plt.savefig('./Mobilenet_V2/conf_net_val.jpg')
...
def main():
    ...
    save_path = './Mobilenet_V2/mobilenet_v2.pth'#模型最终保存路径
    data_transform = {
    ...
    with open('./Mobilenet_V2/class_indices.json', 'w') as json_file:
    ...
    with open('./Mobilenet_V2/acc_time_net_val.txt', 'a+', encoding='utf-8') as f:
    ...
    plt.legend()
    plt.savefig('./Mobilenet_V2/acc_net.jpg')
    plt.show(block=False)
    plt.pause(5)
    plt.close()
    plt.figure(2)
    ...
    plt.legend()
    plt.savefig('./Mobilenet_V2/loss_net.jpg')
    plt.show(block=False)
    plt.pause(5)
    plt.close()
