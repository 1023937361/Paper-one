...
def summary_table(matrix,num_classes,labels):
    ...
    with open('./Mobilenet_V2/table_net_test.txt', 'a+') as f:
        f.write(str(table))
    f.close()
    print(table)
...
def plot_conf(matrix,num_classes,labels):
    ...
    plt.tight_layout()
    plt.savefig('./Mobilenet_V2/conf_net_test.jpg')
...
def main():
    ...
    weights_path = "./Mobilenet_V2/mobilenet_v2.pth"
    ...
    print()
    print("总测试时间：", t2)
    with open('./Mobilenet_V2/acc_time_net_test.txt', 'a+', encoding='utf-8') as f:
