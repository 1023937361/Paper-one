...
def summary_table(matrix,num_classes,labels):
    ...
    with open('./Xception_Padding/table_net-padding_test.txt', 'a+') as f:
        f.write(str(table))
    f.close()
    print(table)
...
def plot_conf(matrix,num_classes,labels):
    ...
    plt.tight_layout()
    plt.savefig('./Xception_Padding/conf_net-padding_test.jpg')
...
def main():
    ...
    weights_path = "./Xception_Padding/xception-padding.pth"
    ...
    with open('./Xception_Padding/acc_time_net-padding_test.txt', 'a+', encoding='utf-8') as f:
