...
def summary_table(matrix,num_classes,labels):
    ...
    with open('./Alexnet/table_net_val.txt', 'a+') as f:
        f.write(str(table))
    f.close()
...
def plot_conf(matrix,num_classes,labels):
    ...
    plt.tight_layout()
    plt.savefig('./Alexnet/conf_net_val.jpg')
...
def main():
    ...
    save_path = './Alexnet/alexnet.pth'#模型最终保存路径
    data_transform = {
    ...
    with open('./Alexnet/acc_time_net_val.txt', 'a+', encoding='utf-8') as f:
    ...
    plt.legend()
    plt.savefig('./Alexnet/acc_net.jpg')
    plt.show(block=False)
    plt.pause(5)
    plt.close()
    ...
    plt.legend()
    plt.savefig('./Alexnet/loss_net.jpg')
    plt.show(block=False)
    plt.pause(5)
    plt.close()
    ...
    # plt.show(block=False)
    # plt.savefig('./conf_net-padding_val'+str(k)+'.jpg')
    # plt.pause(5)
    # plt.close()
