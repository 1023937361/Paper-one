...
def summary_table(matrix,num_classes,labels):
    ...
    with open('./ResNet34_Padding/table_net-padding_test.txt', 'a+') as f:
        f.write(str(table))
    f.close()
    print(table)
...
def plot_conf(matrix,num_classes,labels):
    ...
    plt.tight_layout()
    plt.savefig('./ResNet34_Padding/conf_net-padding_test.jpg')
...
def main():
    ...
    weights_path = "./ResNet34_Padding/resnet34-padding.pth"
    ...
    print()
    print("总测试时间：", t2)
    with open('./ResNet34_Padding/acc_time_net-padding_test.txt', 'a+', encoding='utf-8') as f:
