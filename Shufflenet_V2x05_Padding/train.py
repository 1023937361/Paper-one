...
def summary_table(matrix,num_classes,labels):
    ...
    with open('./Shufflenet_V2x05_Padding/table_net-padding_val.txt', 'a+') as f:
        f.write(str(table))
    f.close()
    print(table)
...
def plot_conf(matrix,num_classes,labels):
    ...
    plt.tight_layout()
    plt.savefig('./Shufflenet_V2x05_Padding/conf_net-padding_val.jpg')
...
def main():
    ...
    save_path = './Shufflenet_V2x05_Padding/shufflenet_v2_x0_5-padding.pth'#模型最终保存路径
    data_transform = {
    ...
    image_path = 'D:\\liuwanli_demo\\Data_aug\\cell_data_padding';
    ...
    with open('./Shufflenet_V2x05_Padding/class_indices.json', 'w') as json_file:
    ...
    net = models.shufflenet_v2_x0_5(num_classes=5,pretrained= False)

    #Inception-v3
    # net = Inception3(num_classes=5, aux_logits=False)
    ...
    with open('./Shufflenet_V2x05_Padding/acc_time_net-padding_val.txt', 'a+', encoding='utf-8') as f:
    ...
    plt.legend()
    plt.savefig('./Shufflenet_V2x05_Padding/acc_net-padding.jpg')
    plt.show(block=False)
    plt.pause(5)
    plt.close()
    ...
    plt.legend()
    plt.savefig('./Shufflenet_V2x05_Padding/loss_net-padding.jpg')
    plt.show(block=False)
    plt.pause(5)
    plt.close()
    #输出混淆矩阵
    ...
    write_excel_xls_append('./standarddata_val.xls',[[p*100,r*100,f*100,round(best_acc*100,2)]])
