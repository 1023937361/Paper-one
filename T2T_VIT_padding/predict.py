...
def summary_table(matrix,num_classes,labels):
    ...
    with open('./T2T_VIT_padding/table_net-padding_test.txt', 'a+') as f:
        f.write(str(table))
    f.close()
    print(table)
...
def plot_conf(matrix,num_classes,labels):
    ...
    plt.tight_layout()
    plt.savefig('./T2T_VIT_padding/conf_net-padding_test.jpg')
...
def main():
    ...
    weights_path = "./T2T_VIT_padding/t2t_vit-padding.pth"
    ...
    print()
    print("总测试时间：", t2)
    with open('./T2T_VIT_padding/acc_time_net-padding_test.txt', 'a+', encoding='utf-8') as f:
