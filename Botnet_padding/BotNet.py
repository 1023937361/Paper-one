...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=1000, resolution=(224, 224)):
        ...
        self.layer4 = self._make_layer(block, 512, num_blocks[3], stride=2,mhsa = True)  # ,mhsa = True
    ...
    def _make_layer(self, block, planes, num_blocks, stride=2, mhsa=False):
...
def ResNet50(num_classes=5, resolution=(224, 224)):
    return ResNet(Bottleneck, [3, 4, 6, 3], num_classes=num_classes, resolution=resolution)


def main():
    device = torch.device("cuda:0")
    model = ResNet50(num_classes=2, resolution=(224, 224)).to(device)###调用方法
