...
def summary_table(matrix,num_classes,labels):
    ...
    with open('./ResNet34/table_net_val.txt', 'a+') as f:
        f.write(str(table))
    f.close()
    print(table)
...
def plot_conf(matrix,num_classes,labels):
    ...
    plt.tight_layout()
    plt.savefig('./ResNet34/conf_net_val.jpg')
...
def main():
    ...
    save_path = './ResNet34/resnet34.pth'#模型最终保存路径
    data_transform = {
    ...
    with open('./ResNet34/class_indices.json', 'w') as json_file:
    ...
    with open('./ResNet34/acc_time_net_val.txt', 'a+', encoding='utf-8') as f:
    ...
    plt.legend()
    plt.savefig('./ResNet34/acc_net.jpg')
    plt.show(block=False)
    plt.pause(5)
    plt.close()
    ...
    plt.legend()
    plt.savefig('./ResNet34/loss_net.jpg')
    plt.show(block=False)
    plt.pause(5)
    plt.close()
