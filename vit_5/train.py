...
from vit_5.vit_pytorch import ViT
...
def summary_table(matrix,num_classes,labels):
    ...
    with open('./vit_5/table_net_val.txt', 'a+') as f:
        f.write(str(table))
    f.close()
    print(table)
...
def plot_conf(matrix,num_classes,labels):
    ...
    plt.tight_layout()
    plt.savefig('./vit_5/conf_net_val.jpg')
...
def main():
    ...
    save_path = './vit_5/vit.pth'#模型最终保存路径
    data_transform = {
    ...
    with open('./vit_5/class_indices.json', 'w') as json_file:
    ...
    with open('./vit_5/acc_time_net_val.txt', 'a+', encoding='utf-8') as f:
    ...
    plt.legend()
    plt.savefig('./vit_5/loss_net.jpg')
    plt.show(block=False)
    plt.pause(5)
    plt.close()
