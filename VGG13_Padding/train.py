...
from VGG13_Padding.model1 import vgg
...
def summary_table(matrix,num_classes,labels):
    ...
    with open('./VGG13_Padding/table_net-padding_val.txt', 'a+') as f:
        f.write(str(table))
    f.close()
    print(table)
...
def plot_conf(matrix,num_classes,labels):
    ...
    plt.tight_layout()
    plt.savefig('./VGG13_Padding/conf_net-padding_val.jpg')
...
def main():
    ...
    save_path = './VGG13_Padding/vgg13-padding.pth'#模型最终保存路径
    data_transform = {
    ...
    with open('./VGG13_Padding/acc_time_net-padding_val.txt', 'a+', encoding='utf-8') as f:
    ...
    plt.legend()
    plt.savefig('./VGG13_Padding/acc_net-padding.jpg')
    plt.show(block=False)
    plt.pause(5)
    plt.close()
    ...
    plt.legend()
    plt.savefig('./VGG13_Padding/loss_net-padding.jpg')
    plt.show(block=False)
    plt.pause(5)
    plt.close()
