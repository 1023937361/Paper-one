...
def summary_table(matrix,num_classes,labels):
    ...
    with open('./Inception_V3/table_net_val.txt', 'a+') as f:
        f.write(str(table))
    f.close()
    print(table)
...
def plot_conf(matrix,num_classes,labels):
    ...
    plt.tight_layout()
    plt.savefig('./Inception_V3/conf_net_val.jpg')
...
def main():
    ...
    save_path = './Inception_V3/Inception_V3.pth'#模型最终保存路径
    data_transform = {
    ...
    with open('./Inception_V3/class_indices.json', 'w') as json_file:
    ...
    # net = models.inception_v3(num_classes=5,pretrained= False)
    ...
    with open('./Inception_V3/acc_time_net_val.txt', 'a+', encoding='utf-8') as f:
    ...
    plt.legend()
    plt.savefig('./Inception_V3/acc_net.jpg')
    plt.show(block=False)
    plt.pause(5)
    plt.close()
    ...
    plt.legend()
    plt.savefig('./Inception_V3/loss_net.jpg')
    plt.show(block=False)
    plt.pause(5)
    plt.close()
