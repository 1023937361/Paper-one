...
def summary_table(matrix,num_classes,labels):
    ...
    with open('./Botnet/table_net_val.txt', 'a+') as f:
        f.write(str(table))
    f.close()
...
def plot_conf(matrix,num_classes,labels):
    ...
    plt.tight_layout()
    plt.savefig('./Botnet/conf_net_val.jpg')
...
def main():
    ...
    save_path = './Botnet/Botnet.pth'#模型最终保存路径
    data_transform = {
    ...
    with open('./Botnet/class_indices.json', 'w') as json_file:
    ...
    with open('./Botnet/acc_time_net_val.txt', 'a+', encoding='utf-8') as f:
    ...
    plt.legend()
    plt.savefig('./Botnet/acc_net.jpg')
    plt.show(block=False)
    plt.pause(5)
    plt.close()
    ...
    plt.legend()
    plt.savefig('./Botnet/loss_net.jpg')
    plt.show(block=False)
    plt.pause(5)
    plt.close()
