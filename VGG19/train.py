...
def summary_table(matrix,num_classes,labels):
    ...
    with open('./VGG19/table_net_val.txt', 'a+') as f:
        f.write(str(table))
    f.close()
    print(table)
...
def plot_conf(matrix,num_classes,labels):
    ...
    plt.tight_layout()
    plt.savefig('./VGG19/conf_net_val.jpg')
...
def main():
    ...
    save_path = './VGG19/vgg19.pth'#模型最终保存路径
    data_transform = {
    ...
    with open('./VGG19/acc_time_net_val.txt', 'a+', encoding='utf-8') as f:
    ...
    plt.legend()
    plt.savefig('./VGG19/loss_net.jpg')
    plt.show(block=False)
    plt.pause(5)
    plt.close()
    #输出混淆矩阵
